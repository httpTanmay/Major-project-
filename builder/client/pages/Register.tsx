...
export default function Register() {
  ...
  const startRegistration = (e: React.FormEvent) => {
    ...
    const registration = { firstName, lastName, email, country };
    localStorage.setItem("registration", JSON.stringify(registration));
    localStorage.setItem("userProfile", JSON.stringify({ fullName: `${firstName} ${lastName}`.trim(), email, country }));
  ...
  const finishOnboarding = (e: React.FormEvent) => {
    ...
    try {
      const current = JSON.parse(localStorage.getItem("userProfile") || "null") || {};
      localStorage.setItem("userProfile", JSON.stringify({ ...current, fullName, email, country: sellerCountry, languages }));
    } catch {}
